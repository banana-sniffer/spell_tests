...
class RBM():
	def __init__(self,visible_nodes,hidden_nodes):
		self.W = torch.randn(nh,visible_nodes)
	...
	# v0: input ratings of all movies by one user
	# vk: visible nodes obtained after K samplings, K iterations and K contrastive divergence
	# ph0: vector of probabilities that at the first iteration the hidden nodes equal one given v0
	# phk: vector of probabilities of the hidden nodes after K samplings given the visible nodes vk
...
for epoch in range(1,nb_epoch+1):
	...
	for id_user in range(0,nb_users-batch_size,batch_size):
		...
		s += 1.
	file.write("epoch: " + str(epoch) + " ,loss: " + str(train_loss/s))
...
for id_user in range(nb_users):
	...
	if len(vt[vt>=0]) > 0:
		...
		s += 1.
file.write("test loss: " + str(test_loss/s))
file.close()
